...
def run_seed():
    ...
    try:
        ...
        assignment_topics = {
            ...
            "CS301": ["Divide and Conquer Analysis", "Dynamic Programming", "Greedy Algorithms"],
            "CS302": ["ER Modeling", "SQL Queries", "Normalization"],
            "CS304": ["Process Scheduling", "Memory Management", "File Systems"],
            "CS401": ["TCP/IP Protocol Suite", "Network Security Basics"],
            "CS501": ["Search Algorithms", "Neural Network Basics"],
            "CS601": ["Linear Regression", "Decision Trees", "Clustering"],
        }
        ...
        for tca_key, tca in tca_map.items():
            ...
            for idx, topic in enumerate(topics[:2]):
                ...
                assignment = Assignment(
                    ...
                    teacher_course_assignment_id=tca.id,
                    teacher_id=tca.teacher_id,
                    title=f"{topic} - Assignment {idx + 1}",
                    description=f"Complete the following problems related to {topic}.",
                    topic=topic,
                    difficulty=rng.choice(["easy", "medium", "hard"]),
                    total_marks=rng.choice([25, 50, 100]),
                    deadline=datetime(deadline.year, deadline.month, deadline.day, 23, 59, tzinfo=timezone.utc),
                    status="published",
                    is_published=True,
                )
                db.add(assignment)
                ...
                for student in section_students:
                    if rng.random() < 0.75:
                        ...
                        sub = AssignmentSubmission(
                            ...
                            student_id=student.id,
                            submitted_at=datetime.now(timezone.utc) - timedelta(days=rng.randint(0, 7)),
                            score=round(score / 100 * assignment.total_marks, 1) if is_graded else None,
                            max_score=assignment.total_marks if is_graded else None,
        ...
        for tca_key, tca in tca_map.items():
            ...
            topics = assignment_topics.get(course_code, ["General Topic"])
            topic = topics[0]
