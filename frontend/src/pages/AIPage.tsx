...
import {
  ...
  Copy, Check, RotateCw, PlusCircle, Trash2, MessageSquare, ChevronDown
} from 'lucide-react';
  ...
  return (
    <div className="flex h-[calc(100vh-7rem)] bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
            ...
            {conversations.length === 0 ? (
              ...
              conversations.map((c) => (
                <button
                  ...
                  className={`w-full text-left px-2.5 py-2 rounded-lg text-xs font-medium truncate flex items-center gap-2 transition-all ${
                    conversationId === c.id
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Header Banner */}
        <div className="bg-gradient-to-r from-[#005BAC] via-[#0A6FD8] to-[#8CC63F] p-4 sm:p-5 text-white shadow-md flex items-center justify-between gap-4 relative overflow-hidden flex-shrink-0">
          <div className="space-y-1 relative z-10">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/20 backdrop-blur-md text-xs font-extrabold text-white">
              <Bot className="w-3.5 h-3.5 text-[#8CC63F]" /> ChatGPT Copilot Workspace
            </div>
            <h1 className="text-lg sm:text-xl font-black">EduPilot AI Copilot</h1>
          </div>
